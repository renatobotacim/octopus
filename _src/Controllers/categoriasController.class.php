...
require_once('../_models/AdminCategorias.class.php');
...
class categoriasController {
    ...
    public function countByEmp($id, $params) {
        $read = new Read;
        $QueryParams = $this->montaQuery($params);
        $read->FullRead("Select categoria_id from inf_categorias WHERE categoria_empresa_id = :id {$QueryParams}", "id={$id}");
    ...
    public function create($Data) {
        ...
        if ($cadastra->getResult()):
            redireciona("painel.php?exe=categorias/update&create=true&id=" . $cadastra->getResult());
        else:
