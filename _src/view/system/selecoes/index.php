...
<section class="content">
    <div class="container-fluid">
        ...
        <a href="painel.php?exe=<?= $navegacao[0] ?>/create">
            <button type="button" name="alterStatus" class="btn btn-success btn-block btn-sm btn-flat mb-3"><i class="fas fa-plus"></i>&nbsp;&nbsp;Cadastrar Processo Seletivo / Concurso</button>
        </a>

        <div class="card card-primary card-outline">
            <!--Título do box-->
            <div class="card-header"><h3 class="card-title">
                    <i class="fas fa-list"></i>&nbsp;&nbsp;Listagem de Serviços</h3>
            </div>
            <div class="">
                    ...
                    <table id="datatable" class="table table-bordered table-striped">
                        ...
                        <tbody id="tabela-titulo">
                            ...
                            if (!empty($Data)):
                                foreach ($Data as $i):
                                    extract($i);
                                    if ($processo_seletivo_tipo == 1):
                                        $processo_seletivo_tipo = "Processo Seletivo";
                                    else:
                                        $processo_seletivo_tipo = "Concurso";
                                    endif;
                                    echo "<tr>"
                                    . "<td>{$processo_seletivo_id}</td>"
                                    . "<td>{$processo_seletivo_tipo}</td>"
                                    . "<td>{$processo_seletivo_numero}-{$processo_seletivo_ano}</td>"
                                    . "<td>{$processo_seletivo_descricao}</td>"
                                    . "<td>{$processo_seletivo_categoria_id}</td>"
                                    . "<td><a class=\"btn btn-flat btn-sm btn-action btn-primary \" href=\"{$processo_seletivo_link}\" title=\"Link da Ferramenta do Processo\"><i class=\"fas fa-edit\"></i></a></td>"
                                    . "<td><a class=\"btn btn-flat btn-sm btn-action btn-primary \" href=\"painel.php?exe={$navegacao[0]}/update&id={$processo_seletivo_id}\" title=\"Alterar Registro\"><i class=\"fas fa-edit\"></i></a>"
                                    . "<a class=\"btn btn-sm btn-flat btn-action btn-danger\" onclick=\"return confirm('Deseja realmente excluir esse registro?')\" type=\"submit\" title=\"Excluir Registro\" href=\"painel.php?exe={$navegacao[0]}/index&delete={$processo_seletivo_id}\"><i class=\"fas fa-trash-alt\"></i></a>"
                                    . "</tr>";
                                endforeach;
                            else:
