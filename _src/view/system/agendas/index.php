    ...
    <div class="container-fluid">
        ...
        <div class="card card-primary card-outline">
            ...
            <div class="card-header"><h3 class="card-title">
                    <i class="fas fa-list"></i>&nbsp;&nbsp;Listagem de Agendas</h3>
            </div>
            <div class="">
                ...
                <div class="card-body">   
                    <table id="datatable" class="table table-bordered table-striped">
                        ...
                        <tbody id="tabela-titulo">
                            <?php
                            if (!empty($Data)):
                                $type = ['r' => 'Reunião', 'v' => 'Viagem', 'e' => 'Evento'];
                                foreach ($Data as $i):
                                    extract($i);
                                    echo "<tr>"
                                    . "<td>{$agenda_id}</td>"
                                    . "<td>{$type[$agenda_tipo]}</td>"
                                    . "<td>{$agenda_ano}</td>"
                                    . "<td>{$agenda_data}</td>"
                                    . "<td>{$agenda_hora}</td>"
                                    . "<td>{$agenda_local}</td>"
                                    . "<td>{$agenda_assunto}</td>"
                                    . "<td>{$agenda_autoridades}</td>"
                                    . "<td>{$categoria_nome}</td>"
                                    . "<td><a class=\"btn btn-flat btn-sm btn-action btn-primary \" href=\"painel.php?exe={$navegacao[0]}/update&id={$agenda_id}\" title=\"Alterar Registro\"><i class=\"fas fa-edit\"></i></a>"
                                    . "<a class=\"btn btn-sm btn-flat btn-action btn-danger\" onclick=\"return confirm('Deseja realmente excluir esse registro?')\" type=\"submit\" title=\"Excluir Registro\" href=\"painel.php?exe={$navegacao[0]}/index&delete={$agenda_id}\"><i class=\"fas fa-trash-alt\"></i></a>"
                                    . "</tr>";
                                endforeach;
                            else:
