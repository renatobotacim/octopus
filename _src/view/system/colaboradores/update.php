...
if (isset($_FILES['colaborador_imagem']) && $_FILES['colaborador_imagem']['size'] != 0):
    $tmp_img = $_FILES['colaborador_imagem'];
else:
    $readimg = new Read();
    $readimg->ExeRead("inf_colaboradores", "WHERE colaborador_id = :id", "id={$id}");
    if ($readimg->getResult()):
        if (!empty($readimg->getResult()[0]['colaborador_imagem'])):
            $tmp_img = $readimg->getResult()[0]['colaborador_imagem'];
        else:
            $tmp_img = ' ';
        endif;
    endif;
...
if (!empty($Data['sendForm'])):
    unset($Data['sendForm']);
    $Data['colaborador_imagem'] = $tmp_img;
    $Data['colaborador_empresa_id'] = $userlogin['usuario_empresa_id'];
    require ('_models/AdminColaboradores.class.php');
    $update = new AdminColaboradores();

    $update->ExeUpdate($id, $Data);
    $link = 'painel.php?exe=colaboradores/index&update=sucess'; // especifica o endereço
    redireciona($link); // chama a função
    IMDErro($update->getError()[0], $update->getError()[1]);
else:
    $read = new Read();
    $read->ExeRead("inf_colaboradores", "WHERE colaborador_id = :id", "id={$id}");
    if (!$read->getResult()):
        header('Location:painel.php?exe=colaboradores/index&empty=true');
    else:
...
if ($checkCreate):
    IMDErro("<i class=\"fa fa-check\" aria-hidden=\"true\"></i><br><b>Sucesso ao Cadastrar!</b><br>O Conteúdo foi criado com sucesso no sistema! <br>Continue atualizando o mesmo.", IMD_ACCEPT);
endif;
                ...
                <div class="box-body">
                    <form name="fileForm" action="" method="post" enctype="multipart/form-data">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <label class="uploadArquivo" for="colaborador_imagem">
                                        <?php
                                        if (!isset($Data['colaborador_imagem']) || ($Data['colaborador_imagem'] == 0)):
                                            ...
                                            echo "<img src=\"./uploads/{$Data['colaborador_imagem']}\" width=\"100%\" heigth=\"auto\" style=\" max-height: 350px;\"";
                                            echo "<img id=\"preview\" width=\"100%\" heigth=\"auto\" style=\"position: relative; top:0;left: 0;max-height: 350px;\">";
                                        endif;
                                        ?>     
                                    </label>
                            ...
                            <div class="col-md-9">
                                ...
                                <div class="col-lg-12">
                                    <div class="form-group">
                                        ...
                                        <textarea type="text" rows="3" name="colaborador_informacoes" id="colaborador_informacoes" class="form-control" placeholder="Insira um comentário do colaborador"><?php if (isset($Data)) echo $Data['colaborador_informacoes']; ?></textarea>
                                    </div>
                                </div>   
                            </div>
                        </div>
                        ...
                        <div class="text-right">
                            <button type="submit" value="Cadastrar" name="sendForm" class="btn btn-success btn-flat"><i class="fa fa-floppy-o" aria-hidden="true"></i>&nbsp;Salvar</button>
