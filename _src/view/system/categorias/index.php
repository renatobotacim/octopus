    ...
    <div class="container-fluid">
        ...
        <div class="card card-primary card-outline">
            ...
            <div class="card-header"><h3 class="card-title">
                    <i class="fas fa-list"></i>&nbsp;&nbsp;Listagem de Categorias</h3>
            </div>
            <div class="">
                    ...
                    <table id="datatable" class="table table-bordered table-striped">
                        ...
                        <tbody id="tabela-titulo">
                            ...
                            if (!empty($Data)):
                                foreach ($Data as $i):
                                    extract($i);
                                    if ($categoria_status == 2):
                                        $categoria_status = ['success', 'ATIVA'];
                                    else:
                                        $categoria_status = ['default', 'INATIVA'];
                                    endif;
                                    echo "<tr>"
                                    . "<td>{$categoria_id}</td>"
                                    . "<td>{$categoria_nome}</td>"
                                    . "<td><button class=\"btn btn-flat btn-block btn-sm btn-{$categoria_status[0]} mx-1 my-1 \" title=\"Status da noticia\">{$categoria_status[1]}</td></td>"
                                    . "<td><a class=\"btn btn-flat btn-sm btn-action btn-primary \" href=\"painel.php?exe={$navegacao[0]}/update&id={$categoria_id}\" title=\"Alterar Registro\"><i class=\"fas fa-edit\"></i></a>"
                                    . "<a class=\"btn btn-sm btn-flat btn-action btn-danger\" onclick=\"return confirm('Deseja realmente excluir esse registro?')\" type=\"submit\" title=\"Excluir Registro\" href=\"painel.php?exe={$navegacao[0]}/index&delete={$categoria_id}\"><i class=\"fas fa-trash-alt\"></i></a>"
                                    . "</tr>";
                                endforeach;
                            else:
