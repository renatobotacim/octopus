...
<section class="content">
    <div class="container-fluid">
        ...
        <a href="painel.php?exe=<?= $navegacao[0] ?>/create">
            <button type="button" name="alterStatus" class="btn btn-success btn-block btn-sm btn-flat mb-3"><i class="fas fa-plus"></i>&nbsp;&nbsp;Criar Notícia</button>
        </a>
                        ...
                        <tbody id="tabela-titulo">
                            ...
                            if (!empty($Data)):
                                foreach ($Data as $i):
                                    ...
                                    $status = ['warning' => 'RASCUNHO!', 'success' => 'PUBLICADA!', 'info' => 'AGENDADA!','danger' => 'OCULTA!'];
                                    $status1 = ['RASCUNHO!', 'PUBLICADA!', 'AGENDADA!','OCULTA!'];

                                    if ($noticia_destaque == 2):
                                        $noticia_destaque = ['success', 'SIM'];
                                    else:
                                        $noticia_destaque = ['default', 'NÃO'];
                                    endif;

                                    echo "<tr>"
                                    . "<td>{$noticia_id}</td>"
                                    . "<td>{$noticia_titulo}</td>"
                                    . "<td>{$noticia_data}<br>{$noticia_data_publicacao}</td>"
                                    . "<td><button class=\"btn btn-flat btn-block btn-sm btn-" . array_keys($status)[$noticia_status - 1] . " mx-1 my-1 \" title=\"Status do Pagamento\">{$status1[$noticia_status - 1]}</td></td>"
                                    . "<td><button class=\"btn btn-flat btn-block btn-sm btn-{$noticia_destaque[0]} mx-1 my-1 \" title=\"Status da noticia\">{$noticia_destaque[1]}</td></td>"
                                    . "<td><a class=\"btn btn-flat btn-sm btn-action btn-primary \" href=\"painel.php?exe={$navegacao[0]}/update&id={$noticia_id}\" title=\"Alterar Registro\"><i class=\"fas fa-edit\"></i></a>"
                                    . "<a class=\"btn btn-sm btn-flat btn-action btn-danger\" onclick=\"return confirm('Deseja realmente excluir esse registro?')\" type=\"submit\" title=\"Excluir Registro\" href=\"painel.php?exe={$navegacao[0]}/index&delete={$noticia_id}\"><i class=\"fas fa-trash-alt\"></i></a>"
                                    . "</tr>";
                                endforeach;
                            else:
